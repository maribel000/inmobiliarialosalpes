...
public class PersistenceServices implements IPersistenceServices {
    ...
    public void llenardatos(int a) {
        for (int i = 0; i < a; i++)
        {
            ...
            if(i == 0){usuario.setTipoUsuario("Administrador");}
            else if ( i % 2 == 0){usuario.setTipoUsuario("Vendedor");}
            else{usuario.setTipoUsuario("Cliente");}
            ...
            if (i % 2 == 0 ){inmueble.setTipo(1+"");}
            else{inmueble.setTipo(2+"");}
            inmueble.setidInmueble(i);

            inmuebles.add(inmueble);

            Transacciones t = new Transacciones();
            if (i % 3 == 0){t.setEstado("Pendiente");}
            else{t.setEstado("Finalizada");}
